...
def clipped_local_structure(coords, k_clip=8, eps=1e-8):
    coords = np.asarray(coords, dtype=np.float64)
    D = euclidean_distances(coords, coords)

    # scale = median nearest-neighbor distance
    D_no_diag = D.copy()
    np.fill_diagonal(D_no_diag, np.inf)
    s = np.median(np.min(D_no_diag, axis=1))
    s = max(s, eps)

    # automatic clipping radius from local geometry
    k_clip = min(k_clip, coords.shape[0] - 1)
    kth = np.partition(D_no_diag, k_clip, axis=1)[:, k_clip]
    tau = np.median(kth) / s

    C = np.minimum(D / s, tau)
...
def pairwise_align(
    ...
    **kwargs) -> Union[NDArray[np.floating], Tuple[NDArray[np.floating], float, float, float, float]]:
    ...
    C1_np = clipped_local_structure(sliceA.obsm["spatial"])
    C2_np = clipped_local_structure(sliceB.obsm["spatial"])

    D_A = nx.from_numpy(C1_np)
    D_B = nx.from_numpy(C2_np)
